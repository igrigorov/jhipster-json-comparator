...
public class WorkflowCompareService {
	...
	public CompareResponse singleCompare(ComPair input) {
		...
		var dbRequests = requestsRepository.findByRequestIdAndRequestIdxAndSrcSystemIn(requestId, idx,
				Arrays.asList(input.getSystem1(), input.getSystem2()));

		LOG.info("{}Retrieved dbRequests: {}", pfx, dbRequests.toString());

		if (dbRequests.size() != 2)
			throw new ComparisonException("Invalid number of dbRequests found in DB: expected 2, found " + dbRequests.size());
